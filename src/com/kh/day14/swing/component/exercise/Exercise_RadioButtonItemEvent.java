...
public class Exercise_RadioButtonItemEvent extends JFrame {
	...
	private ImageIcon[] images = { new ImageIcon("images/apple.jpg"), new ImageIcon("images/pear.jpg"),
			new ImageIcon("images/cherry.jpg") }; // 이미지 라벨에 들어갈 이미지아이콘 배열
	...
	public Exercise_RadioButtonItemEvent() {
		...
		JPanel radioJPanel = new JPanel(); // 라디오 버튼을 추가할 팬을 만듬
		radioJPanel.setBackground(Color.LIGHT_GRAY); // 새로만든 팬의 배경색을 바꿈
		...
		for (int i = 0; i < fruits.length; i++) {
			...
			radioJPanel.add(fruits[i]); // 새로 만든 라디오 팬에 추가함
...
//					}
					if(fruits[0].isSelected()) {
						imageLabel.setIcon(images[0]); // setIcon() 메소드, Label 이미지를 Set해주는 메소드
		...
		c.add(radioJPanel, BorderLayout.NORTH); // 라디오팬을 컨텍트팬에 추가함. 탑으로 붙여서 보일거임
